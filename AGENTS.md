...
- `docs/`: 複数ファイル・複数ツールにまたがるワークフローガイドと設計判断を置きます。単一ファイル内で完結する判断は、そのファイルの「なぜ」コメントに書きます。
...
ドキュメントはすべて日本語で書きます。コードの再説明は書かず、単一ファイル内の判断は「なぜ」コメント、ファイルやツールをまたぐ判断だけを `docs/` に置きます。

鮮度規約: `nix/modules/home/programs/herdr.nix`、`fish/conf.d/fzf-widgets.fish`、`fish/functions/fzf_command_palette.fish` のキー割り当てを変更するコミットでは、`docs/cheatsheet.md` を同時に更新してください。表は「キー・操作名・一行の用途」に限定し、挙動の詳細を書かないことで更新コストを抑えます。

ワークフロー上の制約: worktree の作成は gtr 経由で行い、herdr のネイティブ作成は使いません。背景と運用ルールは `docs/worktree.md` を参照してください。
